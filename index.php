<?php
// เริ่มต้น Session และเชื่อมต่อฐานข้อมูลเพื่อดึง Config มาแสดงผลใน Title/Logo
session_start();
require_once 'db.php';

// ค่า Default
$appName = 'ระบบ ปพ.5 และ ปพ.6';
$schoolName = 'โรงเรียนประถมศึกษา';
$schoolLogoUrl = '';

try {
    // ดึง Config จากตาราง
    $stmt = $pdo->prepare("SELECT setting_value FROM config WHERE setting_key = 'main_config'");
    $stmt->execute();
    $row = $stmt->fetch();
    ...
    if ($row) {
        ...
        $schoolName = $config['school_name'] ?? $schoolName;
        $schoolLogoUrl = $config['school_logo_url'] ?? '';
    }
} catch (Exception $e) {
    // กรณีติดต่อ Database ไม่ได้ ใช้ค่า Default
}
?>
<!DOCTYPE html>
<html lang="th">
<head>
  ...
  <title><?php echo htmlspecialchars($appName); ?> | <?php echo htmlspecialchars($schoolName); ?></title>
  ...
  <!-- jQuery & Thailand.js (สำหรับที่อยู่) -->
...
<body class="bg-gray-50 transition-colors duration-200">
      ...
      <div class="relative flex flex-col items-center">
          <div class="relative mb-6">
              <?php if (!empty($schoolLogoUrl)) { ?>
                  <div class="w-28 h-28 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-xl animate-bounce-gentle relative z-10 p-3 overflow-hidden border-4 border-blue-50 dark:border-gray-700">
                      <img src="<?php echo htmlspecialchars($schoolLogoUrl); ?>" alt="School Logo" class="w-full h-full object-contain">
                  </div>
              <?php } else { ?>
          ...
          <h3 id="loadingTitle" class="text-2xl font-bold text-gray-800 dark:text-white mb-2 tracking-wide">
              กำลังเข้าสู่ระบบ...
          </h3>
          <p id="loadingText" class="text-gray-500 dark:text-gray-300 text-sm font-light animate-pulse">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 border border-gray-100 relative overflow-hidden">
      ...
      <div class="text-center mb-8">
        <div id="logoContainer" class="inline-block bg-blue-50 rounded-full p-4 mb-4">
          <?php if (!empty($schoolLogoUrl)) { ?>
            <img src="<?php echo htmlspecialchars($schoolLogoUrl); ?>" alt="Logo" class="w-20 h-20 rounded-full object-contain">
          <?php } else { ?>
            <i class="fas fa-school text-5xl text-blue-600"></i>
          <?php } ?>
        </div>
        
        <h1 class="text-3xl font-bold text-gray-800 mb-2"><?php echo htmlspecialchars($appName); ?></h1>
        <p class="text-gray-600"><?php echo htmlspecialchars($schoolName); ?></p>
      </div>

      <div class="flex gap-2 mb-6 bg-gray-100 p-1 rounded-lg">
        <button 
          type="button"
          onclick="switchLoginTab('teacher')"
          class="flex-1 py-2 px-4 rounded-md font-medium transition-colors teacher-tab active bg-white text-blue-600 shadow-sm"
        >
          <i class="fas fa-chalkboard-user mr-2"></i>ครู / ผู้ดูแลระบบ
        </button>
        <button 
          type="button"
          onclick="switchLoginTab('student')"
          class="flex-1 py-2 px-4 rounded-md font-medium transition-colors student-tab text-gray-600 hover:text-gray-800"
        >
      ...
      <form id="teacherLoginForm" class="space-y-6 teacher-form">
        ...
        <div>
          ...
          <div class="relative">
            <input 
              ...
              id="teacher_password" 
              required
              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-10 transition-all"
              placeholder="กรอก Password"
            >
            <button 
              type="button"
              onclick="togglePasswordVisibility('teacher_password', 'icon_teacher_pass')"
        ...
        <button 
          type="submit"
          onclick="handleTeacherLogin(event)"
          class="w-full bg-blue-600 text-white py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2 shadow-md hover:shadow-lg"
        >
      ...
      <form id="studentLoginForm" class="space-y-6 student-form hidden">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">
            <i class="fas fa-id-card mr-2"></i>รหัสนักเรียน
          </label>
          <input 
            type="text"
            id="student_id" 
            ...
            placeholder="กรอก รหัสนักเรียน"
          >
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">
            <i class="fas fa-lock mr-2"></i>เลขประจำตัวประชาชน
          </label>
          <div class="relative">
            <input 
              type="password" 
              id="student_id_card" 
              required
              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent pr-10 transition-all"
              placeholder="กรอก เลขประจำตัวประชาชน"
            >
            <button 
              type="button"
              onclick="togglePasswordVisibility('student_id_card', 'icon_student_pass')"
        ...
        <button 
          type="submit"
          onclick="handleStudentLogin(event)"
          class="w-full bg-green-600 text-white py-3 rounded-lg font-medium hover:bg-green-700 transition-colors flex items-center justify-center space-x-2 shadow-md hover:shadow-lg"
        >
          <i class="fas fa-sign-in-alt"></i>
          <span>เข้าสู่ระบบ</span>
        </button>
      ...
      <div id="teacherHelpText" class="mt-6 text-center text-sm text-gray-500 teacher-form">
        <p>Username เริ่มต้น: <span class="font-semibold">admin</span></p>
        <p>Password เริ่มต้น: <span class="font-semibold">Admin@123456</span></p>
      </div>

      <div id="studentHelpText" class="mt-6 text-center text-sm text-gray-500 student-form hidden">
        <p class="text-blue-600 bg-blue-50 py-2 px-4 rounded-lg inline-block">
          <i class="fas fa-info-circle mr-2"></i>กรอกรหัสนักเรียนและเลขประจำตัวประชาชน
        </p>
      </div>
  ...
  <div id="mainApp" class="hidden">
      ...
      <div class="px-4 md:px-6 h-full flex items-center justify-between">
        ...
        <div class="flex items-center space-x-3 md:space-x-4">
          ...
          <div class="flex items-center space-x-3">
            <?php if (!empty($schoolLogoUrl)) { ?>
              <img src="<?php echo htmlspecialchars($schoolLogoUrl); ?>" alt="Logo" class="w-8 h-8 md:w-10 md:h-10 rounded-lg object-contain">
            <?php } else { ?>
            ...
            <?php } ?>
            <div>
              <h1 class="text-lg md:text-xl font-bold text-gray-800 dark:text-white leading-tight"><?php echo htmlspecialchars($appName); ?></h1>
              <p class="text-xs text-gray-500 dark:text-gray-400 hidden md:block"><?php echo htmlspecialchars($schoolName); ?></p>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center space-x-1 md:space-x-2">
          ...
          <button 
            onclick="toggleTheme()" 
            class="p-2.5 text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-yellow-400 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-300 focus:outline-none transform hover:scale-110"
          ...
          <div class="relative">
            <button 
              ...
              class="relative p-2.5 text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-white rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors focus:outline-none"
            ...
            <div id="notificationDropdown" class="hidden absolute right-0 mt-3 w-80 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden transform origin-top-right transition-all duration-200">
              <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-700">
                <h3 class="font-bold text-gray-800 dark:text-white text-sm">การแจ้งเตือน</h3>
                <button onclick="markAllAsRead()" class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">อ่านทั้งหมด</button>
              </div>
              <div id="notificationList" class="max-h-80 overflow-y-auto">
                <div class="p-4 text-center text-gray-500 dark:text-gray-400 text-sm">กำลังโหลด...</div>
              </div>
              <div class="border-t border-gray-100 dark:border-gray-700 p-2 text-center">
                <a href="#" class="text-xs text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">ดูทั้งหมด</a>
              </div>
            </div>
          </div>
          ...
          <div class="relative">
            ...
            <div id="profileDropdown" class="hidden absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden transform origin-top-right transition-all duration-200">
              <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 border-b border-gray-100 dark:border-gray-600 md:hidden">
                <p class="text-sm font-semibold text-gray-800 dark:text-white">Admin</p>
                <p class="text-xs text-gray-500 dark:text-gray-400">ผู้ดูแลระบบ</p>
              </div>
              <div class="py-1">
    ...
    <div class="flex pt-16">
      ...
      <aside 
        id="sidebar" 
        class="fixed top-0 left-0 h-screen z-50 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 overflow-y-auto -translate-x-full transition-transform duration-300 ease-in-out md:translate-x-0 md:top-16 md:h-[calc(100vh-64px)] md:shadow-none shadow-2xl"
      >
      </aside>
      ...
      <main id="contentArea" class="flex-1 p-4 md:p-6 min-h-screen transition-all duration-300 ease-in-out md:ml-64 bg-gray-50 dark:bg-gray-900">
        ...
        <div id="pageContent" class="max-w-[98%] mx-auto">
        </div>
        ...
        <footer class="mt-12 py-6 text-center text-gray-400 dark:text-gray-500 text-sm border-t border-gray-200 dark:border-gray-700">
          <p>&copy; <?php echo date('Y') + 543; ?> <?php echo htmlspecialchars($schoolName); ?>. All rights reserved.</p>
          <p class="text-xs mt-1">System v2.0.0</p>
        </footer>
      </main>
  ...
  <script>
    window.webAppUrl = window.location.href;
  </script>

  <!-- JavaScript Files -->
  <!-- Utils & Components -->
  <script src="js/js-utils.js"></script>
  <script src="js/js-components.js"></script>
  <script src="js/js-pages.js"></script>
  
  <!-- Page Logic -->
  <script src="js/js-pages-attendance.js"></script>
  <script src="js/js-pages-behaviors.js"></script>
  <script src="js/js-pages-activities.js"></script>
  <script src="js/js-pages-gradesview.js"></script>
  <script src="js/js-pages-pp5.js"></script>
  <script src="js/js-pages-pp6.js"></script>
  <script src="js/js-pages-reports.js"></script>
  <script src="js/js-pages-reading.js"></script>
  <script src="js/js-pages-timetable.js"></script>
